...
Packaging navíc obsahuje `/usr/local/bin/kvazi-healthcheck.php` mimo document root: přes dostupné PHP kontroluje interní HTTP `/healthz`. Umožňuje jednoduchý Coolify CMD bez inline shell/PHP syntaxe, kterou aktuální Coolify validator odmítá; nepřidává HTTP klient ani DB startup hook. Přesný operator příkaz a source omezení popisuje výše odkazovaný provozní kontrakt.
